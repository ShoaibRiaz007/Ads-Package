...
namespace SH.API
{
    public static partial class Extension
    {
        private class CacheItem
        {
            ...
            public CacheItem(GameObject reference)
            {
        ...
        private static readonly Dictionary<int, CacheItem> CACHE = new Dictionary<int, CacheItem>();
        ...
        /// <returns></returns>
        public static T GetInParent<T>(this Component component) where T : Component
        {
            var tem = component.GetComponentInParent<T>();
            if (tem)
            {
                return Get(tem.gameObject, typeof(T)) as T;
            }
        ...
        {
            var tem = gameObject.GetComponentInParent<T>();
            if (tem)
            {
                return Get(tem.gameObject, typeof(T)) as T;
            }
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public static T GetInChildren<T>(this Component component) where T : Component
        {
            var tem = component.GetComponentInChildren<T>();
            if (tem)
            {
                return Get(tem.gameObject, typeof(T)) as T;
            }
        ...
        {
            var tem = gameObject.GetComponentInChildren<T>();
            if (tem)
            {
                return Get(tem.gameObject, typeof(T)) as T;
            }
        ...
        public static Component Get(this GameObject gameObject, Type type)
        {
            ...
            if (!CACHE.TryGetValue(instanceID, out CacheItem cache))
            {
                cache = new CacheItem(gameObject);
        ...
        static void Prune()
        {
            ...
            foreach (KeyValuePair<int, CacheItem> entry in CACHE)
            {
